...
## 209 members favor impeachment, 26 do not
...
impeach %>% 
  group_by(for_impeachment, district) %>% 
  summarize(average = mean(pct_bachelors))
...
impeach %>% 
  group_by(for_impeachment, district) %>% 
  summarize(average = mean(trump_percent))
```
...
## the variable margin_flag_2018 includes those that won by 5 points of less and more
```
...
```{r}
## Find the mean *percentage of the vote that Donald Trump received* for districts that are Yes for impeachment vs. No. that have a predominantly nonwhite population.

impeach %>% 
  filter(pct_nonwhite_compared_to_national == "ABOVE") %>% 
  group_by(for_impeachment, district) %>% 
  summarize(average = mean(trump_percent))
...
```{r}
impeach <- impeach %>% 
  mutate(year_col = year(date_announced),
         month_col = month(date_announced))
